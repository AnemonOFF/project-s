...
$(function () {
    ...
    $(".modal-course__blocks").on("click", ".modal-course__block", function(e){
        ...
        if($("#Block" + block_id).next().is('div.modal-course__block__info'))
...
function CheckPaginationButtons(){
    if(current_page > 0)
        $('.mainbutton.fir').css('visibility', 'visible');
    else
        $('.mainbutton.fir').css('visibility', 'hidden');
    if(current_page + 1< max_page)
        $('.mainbutton.sec').css('visibility', 'visible');
    else
        $('.mainbutton.sec').css('visibility', 'hidden');
}
...
function ShowBlockInfo(block_id, block_info){
    ...
    block_info['tasks'].forEach((task) => {
        tasks_rows.append($('<tr>')
            .append($('<td>', {text: task['name']}))
            .append($('<td>', {text: task['points_student']}))
            .append($('<td>', {text: task['points_max']}))
            .append($('<td>', {text: Math.round(task['points_student'] / task['points_max'] * 100)}))
        );
    });
    block.after($('<div>', {class: 'modal-course__block__info'})
        .append($('<table>')
            .append($('<thead>')
                .append($('<tr>')
...
function GetCourseBlocks(course_id, student_id = null){
    return Promise.resolve(
        $.ajax({
            url: `/api/courses/${course_id}/blocks${!!student_id ? '?student_id=' + student_id : ''}`,
            method: "GET",
        })
    );
}
...
function GetBlockTasks(block_id, student_id = null){
    return Promise.resolve(
        $.ajax({
            url: `/api/blocks/${block_id}/tasks${!!student_id ? '?student_id='+student_id : ''}`,
            method: "GET"
        })
    );
}
...
function ShowCourseBlocks(course_id, student_id){
    ...
    $.ajax({
        ...
        method: "GET",
        success: (data) => {
            $(".modal-course__title h1").text(`${data.full_name} - ${course.getAttribute('data-name')}`);
            student = data;
        },
    });
    GetCourseBlocks(course_id, student_id).then(async function(data){
        blocks = data;
        data.forEach(await async function(block){
            //block['tasks'] = await GetBlockTasks(block['id'], student_id);
            let points_student = 0;
            block['tasks'].forEach((task) => {points_student += task['points_student'];});
            blocks_section.append($('<button>', {
                ...
                .append($('<div>', {
                    ...
                    style: `right: ${100 - points_student / block['points_max'] * 100}%;`
                }))
                .append($('<div>', {
...
function ApplyCourses(event) {
    ...
    GetCourseInfo(course_id, 0).then((data) =>{
        ...
        $(".modal").fadeOut();
        $("body").css("overflow", "auto");
        setTimeout($("#CourseStudentsTableName")[0].scrollIntoView({ behavior: 'smooth', block: 'start'}), 1000);
    }
    );
}
...
function DrawTable(max_points, students) {
    ...
    if($("#CourseStudentsTableName").length == 0)
        $("#CourseStudentsTable").before($("<h1>", {text: course.getAttribute('data-name'), id: 'CourseStudentsTableName', class: 'table__name'}));
    else
        $("#CourseStudentsTableName").text(course.getAttribute('data-name'));
}
